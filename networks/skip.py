...
def skip(
    ...
    model = nn.Sequential()
    model_tmp = model#model_tmp和model地址共享
    ...
    for i in range(len(num_channels_down)):
        ...
        if num_channels_skip[i] != 0:
            skip.add(conv(input_depth, num_channels_skip[i], filter_skip_size, bias=need_bias, pad=pad))#卷积
            skip.add(bn(num_channels_skip[i]))#batch_norm
            skip.add(act(act_fun))#激活
